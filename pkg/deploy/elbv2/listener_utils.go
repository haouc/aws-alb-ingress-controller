...
func buildSDKActions(modelActions []elbv2model.Action, specifyTGArn bool) ([]*elbv2sdk.Action, error) {
	...
	if len(modelActions) != 0 {
		...
		for index, modelAction := range modelActions {
			sdkAction, err := buildSDKAction(modelAction, specifyTGArn)
...
func buildSDKAction(modelAction elbv2model.Action, specifyTGArn bool) (*elbv2sdk.Action, error) {
